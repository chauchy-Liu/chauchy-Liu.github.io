...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var targetEncoding = saveToLocal.get(targetEncodingCookie) === undefined ? defaultEncoding : Number(saveToLocal.get('translate-chn-cht'));
  var translateButtonObject;
  var isSnackbar = GLOBAL_CONFIG.Snackbar !== undefined;
  function translateText(txt) {
    if (txt === '' || txt == null) return '';
    if (currentEncoding === 1 && targetEncoding === 2) return Simplized(txt);else if (currentEncoding === 2 && targetEncoding === 1) {
      return Traditionalized(txt);
    } else return txt;
  }
  function translateBody(fobj) {
    var objs;
    if (_typeof(fobj) === 'object') objs = fobj.childNodes;else objs = document.body.childNodes;
